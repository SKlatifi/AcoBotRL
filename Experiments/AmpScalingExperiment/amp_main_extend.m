function amp_main_extend
    simulate = 0; % 1 = generated random images, 0 = get images over http
    id = '750umGlass_20180502_amptest'; % Identifier of the experiment run
    ...
    default_amp = 0.005; % starting amplitude: for 2*3*5 actuators = 0.02; for 2*3*20 actuators = 0.005
    ...
    datafile = [getTempDataPath() 'AmptestUW2018_20_1_extend\' id '.mat'];
    tmpscalemat = basescale' * (2.^(-10:10));
    tmpscale = reshape(tmpscalemat,1,numel(tmpscalemat));    
    expfreq = tmpscale((tmpscale >= minfreq1 & tmpscale <= maxfreq1)|...
    ...
    for i = 1:N
        if isnan(amps(i)) 
            beforefile = sprintf('%sAmptestUW2018_20_1_extend\\%s_%06d_%.0f_%.0f_in.jpg',getTempDataPath(),id,i,expfreq(exps(i)),duration);       
            afterfile = sprintf('%sAmptestUW2018_20_1_extend\\%s_%06d_%.0f_%.0f_out.jpg',getTempDataPath(),id,i,expfreq(exps(i)),duration);
